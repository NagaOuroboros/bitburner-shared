...
Of note, due to the way Bitburner's filesystem works, the database directory will only exist while it contains files.  If all files are deleted, the directory will 'disappear' from the filesystem; with an exception where orphaned temp files may still persist due to an untimely crash (in which case `{database-root}/tmp/*` will still exist and contain files).
